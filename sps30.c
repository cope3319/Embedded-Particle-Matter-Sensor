...
int16_t start_measurement(){
    struct sensiron_shdlc_rx_header rx_header;
    uint8_t mes_strt = (SPS_MEASURE_START);
    uint8_t len = sizeof(mes_strt);
   /* ret = */return sensiron_shdlc_xvc(SPS_ADDRESS, SPS_MEASURE_START,len, SUB_CMD_START,0,&rx_header, NULL);
/*if(ret != 0){
    return -1;
    fprintf("Start Measurment Error");
}
return 0;
*/
}
...
int16_t stop_measurment(){
    struct sensiron_shdlc_rx_header rx_header;
    return sensiron_shdlc_xvc(SPS_ADDRESS, SPS_MEASURE_STOP,0, NULL,0,&rx_header, NULL);
}

int16_t sps_reset(){
    return sensiron_shdlc_tx(SPS_ADDRESS, SPS_RESET,0, NULL,);
}

int16_t sps_start_cleaning(){
    struct sensiron_shdlc_rx_header rx_header;
    return sensiron_shdlc_xvc(SPS_ADDRESS, SPS_START_FANCLEAN,0, NULL,0,&rx_header, NULL);
}
...
int16_t sps_read_auto_clean_int(uint32_t *interval){
    ...
    int16_t err = 0;
    err = sensiron_shdlc_xvc(SPS_ADDRESS, SPS_FANCLEAN_INT, sizeof(sub_cmd),sub_cmd,sizeof(*interval),&rx_header, (uint8_t *)interval);
...
int16_t sps_write_auto_clean_int(uint32_t interval){
    ...
    return sensiron_shdlc_xvc(SPS_ADDRESS, SPS_FANCLEAN_INT, sizeof(length),(const uint8_t *)length/*pointer to data*/,sizeof(interval),&rx_header,(uint8_t *) &interval/*pointer to the address of interval*/); //use pointer so we can use interval in other places
}
...
uint32_t val;
int16_t err;
err = sensiron_shdlc_xvc(SPS_ADDRESS, SPS_MEASURE_READ,0,NULL,sizeof(data),&rx_header,(uint8_t *) data);

if(err != 0)
return err;

if(sizeof(data) != header.data_len){
    //error message here
}
...
err = sensiron_shdlc_xvc(SPS_ADDRESS, SPS_DEV_INFO,sizeof(sub_cmd),sub_cmd,32,&rx_header,(uint8_t *) serial);
...
err = sensiron_shdlc_xvc(SPS_ADDRESS, SPS_DEV_INFO,sizeof(sub_cmd),sub_cmd,32,&rx_header,(uint8_t *) article_code);
...
err = sensiron_shdlc_xvc(SPS_ADDRESS, SPS_DEV_INFO,sizeof(sub_cmd),sub_cmd,32,&rx_header,(uint8_t *) product_name);
