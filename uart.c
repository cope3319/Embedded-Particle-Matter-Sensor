...
int16_t uart_init(const EUSCI_config config) {
    EUSCI_A0 -> CTLW0 |= (0x0001);                // Set UCSWRST High
    //Initialize all eUSCI_A registers in one
    EUSCI_A0 -> CTLW0 = (basicCTLW0 + config->parityEn + config->MSB + config->bits + 
                              config->stopBit + config->eUSCImode + config->sync +
                              config->clockSel); 
    if(config->baudN < 16) {                    // Set Baud Rate
        EUSCI_A0 -> MCTLW = config->baudE; 
        EUSCI_A0 -> BRW = config->baudN;
    }
    else {
        EUSCI_A0 -> BRW = config->baudN;
        EUSCI_A0 -> MCTLW = 0b1 + config->baudE + 0x60; //oversampling enabled
    }
