...
int config_sps_uart(void){
    ...
    //baud rate config
    EUSCI_A2->BRW = 8; //clock prescaler
    EUSCI_A2->MCTLW |= (EUSCI_A_MCTLW_BRS_MASK & 0xD600); //set secondModReg to 214 (0xD6)

    EUSCI_A2->CTLW0 &= ~(EUSCI_A_CTLW0_SWRST); // Clear software reset

    EUSCI_A2->IE |= 0b11; // Transmit, and Recieve interrupt enabled
    EUSCI_A2->IFG = 0;      // Clear all EUSCI_A2 interrupts
    return 0;
}
...
void sps_uart_send(uint16_t len, uint8_t *data){
    uint16_t count = 0;
    while(count <= len){
        while(EUSCI_A2->IFG < 2); // wait for transmit interrupt flags to clear
